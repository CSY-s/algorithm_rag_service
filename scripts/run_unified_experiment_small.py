...
def create_realistic_test_set():
    ...
    simple_samples = fetch_eval_samples(limit=10, algorithm_id=None)
    ...
    hard_samples = []
    for algo in random.sample(all_algos, min(5, len(all_algos))):
    ...
    return {
        'simple': simple_samples[:3],  # 只取3个
        'medium': medium_samples[:3],  # 只取3个
        'hard': hard_samples[:3],      # 只取3个
        'total': 9
    }
...
def run_experiment_by_difficulty():
    ...
    # 保存JSON
    json_file = output_dir / f'unified_experiment_small_{timestamp}.json'
...
def generate_markdown_report(results, test_set, timestamp, output_dir):
    ...
    md_file = output_dir / f'unified_experiment_small_{timestamp}.md'
    
    with open(md_file, 'w', encoding='utf-8') as f:
        f.write(f"# 统一实验报告(小规模验证)\n\n")
        ...
        for difficulty in ['simple', 'medium', 'hard']:
            ...
            f.write(f"| 方法 | ROUGE-L | BLEU-4 | 概念覆盖 | 解释清晰度 |\n")
            f.write(f"|------|---------|--------|----------|------------|\n")
            
            for method_name, result in difficulty_results.items():
                metrics = result['avg_metrics']
                if metrics:
                    f.write(f"| {method_name} | {metrics['rouge_l']:.4f} | {metrics['bleu_4']:.4f} | {metrics['concept_coverage']:.4f} | {metrics['explanation_clarity']:.4f} |\n")
